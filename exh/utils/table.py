...
class Table:
    ...
    def print_plain(self):
        ...
        rows_and_headers = ([self.header] if self.header is not None else []) + [
            row for row in self.rows if isinstance(row, list)
        ]
        widths = [
            max(len(row[i]) for row in rows_and_headers) + 2
            for i in range(len(rows_and_headers[0]))
        ]
